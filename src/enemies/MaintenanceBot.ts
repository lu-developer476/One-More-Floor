...
export class MaintenanceBot extends EnemyActor {
  ...
  debug(): EnemyDebugState { const body = this.sprite.body as Phaser.Physics.Arcade.Body; return { id: this.bot.id, kind: this.bot.kind, state: this.state, active: this.active, dangerous: this.contactDangerous, contactDangerous: this.contactDangerous, attacking: this.attacking, canBeDisabled: this.canBeDisabled, cameraActive: this.cameraActive, x: this.sprite.x, y: this.sprite.y, bodyX: body.x, bodyY: body.y, velocityX: body.velocity.x, direction: this.direction, stateRemainingMs: 0, detected: false, lineOfSight: false, visibleToCamera: this.cameraActive, visualBounds: { x: this.sprite.x - this.sprite.displayWidth / 2, y: this.sprite.y - this.sprite.displayHeight / 2, width: this.sprite.displayWidth, height: this.sprite.displayHeight }, bodyBounds: { x: body.x, y: body.y, width: body.width, height: body.height } }; }
}
